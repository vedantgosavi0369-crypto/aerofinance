...
export const useFinanceStore = create<FinanceStore>((set, get) => ({
  ...
  addTransaction: async (t) => {
    ...
    if (roundup > 0 && t.type === 'expense') {
      const { data: saveData } = await supabase
        .from('savings_vault')
        .insert([{ amount: roundup, source_expense_id: data.id, user_id: userId }])
        .select()
        .single()
      if (saveData) {
  ...
  addDebt: async (d) => {
    ...
    if (!userId) return
    const supabase = getSupabase()
    const { data } = await supabase.from('debts').insert([{ ...d, user_id: userId }]).select().single()
  ...
  addSubscription: async (s) => {
    ...
    if (!userId) return
    const supabase = getSupabase()
    const { data } = await supabase.from('subscriptions').insert([{ ...s, user_id: userId }]).select().single()
  ...
  processSubscriptions: async () => {
    ...
    for (const sub of toProcess) {
      ...
      await supabase.from('subscriptions').update({ next_due: nextDueStr }).eq('id', sub.id)
  ...
  withdrawSavings: async () => {
    ...
    if (!userId) return
    const supabase = getSupabase()
    await supabase.from('savings_vault').delete().eq('user_id', userId)
    const { data: txData } = await supabase
      .from('transactions')
      .insert([{
        type: 'income',
  ...
  getTotalIncome: () =>
    get().transactions.filter((t) => t.type === 'income').reduce((acc, t) => acc + t.amount, 0),
  getTotalExpenses: () =>
    get().transactions.filter((t) => t.type === 'expense').reduce((acc, t) => acc + t.amount, 0),
  getTotalBorrowed: () =>
    get().debts.filter((d) => d.type === 'borrowed' && !d.settled).reduce((acc, d) => acc + d.amount, 0),
  getTotalLended: () =>
    get().debts.filter((d) => d.type === 'lended' && !d.settled).reduce((acc, d) => acc + d.amount, 0),
  getTotalBalance: () => {
  ...
  getAvgDailySpend: () => {
    ...
    const daysDiff = Math.max(
      1,
      Math.ceil((new Date().getTime() - parseISO(earliest).getTime()) / (1000 * 60 * 60 * 24))
    )
